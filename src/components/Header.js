...
//3. jsx
//4. language selction and updating it in redux store
const Header = () => {
  //give absolute css for header to overlap with body compo
  ...
  // 1.SIGN OUT logic
  const handleSignOut = () => {
    //gets called when user clciks on signout button
    signOut(auth)
      .then(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      //onauthstate..event has unsubscribe() fn from docs which will be returned(API call)
      ...
      if (user) {
        ...
        dispatch(
          //then dispatched to the redux store using adduser action which updates the users data in redux store ...reducer fn(action)
          addUser({
            uid: uid,
            email: email,
  ...
  //redux ke lang ko update kr dega e.target.value se
  const handleLangChange=(e)=>{//dispatches the changeLanguage action with the newly selected language identifier
    //This action updates the lang state in the Redux store, triggering a re-render of components that depend on the selected language
    console.log(e.target.value);
    dispatch(changeLanguage(e.target.value));//update kr denge language ki value current selected value se
  }

  //3. Rendering
  return (
    <div className="absolute w-full px-8 py-2 z-10 flex justify-between">
      <img className="w-48 cursor-pointer" src={LOGO_URL} alt="logo" />

      {user && ( // if we have a user then only show image and signout button
        <div className="flex p-2">
          <select className="bg-red-800 rounded-lg text-white " onChange={handleLangChange}>
          ...
          <button onClick={handleSearch} className="px-4 py-2 mx-4 my-2  bg-red-500 text-white rounded-md">
            GPT Search
          </button>
          <img className="w-12 h-12" src={user.photoURL} alt="user" />
          <button
            onClick={handleSignOut}
            className="font-bold p-2 m-3 text-white"
          >
            Sign Out
          </button>
        </div>
